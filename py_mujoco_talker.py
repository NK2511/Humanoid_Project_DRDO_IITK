...
# 1. Construct absolute path relative to this script file
current_dir = os.path.dirname(os.path.abspath(__file__))
relative_path = "xmls/kondo_scene_flat_ground.xml"
xml_path = os.path.join(current_dir, relative_path)
if not os.path.exists(xml_path):
    xml_path = f"/home/nandhith/humanoid_python/{relative_path}"
...
def main():
    ...
    ros = MujocoRosConnector(node_name='mujoco_talker')
    ...
    if GENERATE_VIDEO:
        ...
        with mujoco.viewer.launch_passive(model, data) as viewer:
            ...
            while viewer.is_running() and time.time() - start_time < 20:
                ...
                # --- ROS Publishing ---
                ros.publish('qpos', data.qpos)
                ros.publish('qvel', data.qvel)
                ros.publish('time', data.time)
                # ----------------------
